...
def render_rdc_summary(rdc_id, requirements):
    ...
    st.markdown(f"""
    <div style="background: rgba(52,152,219,0.05); padding: 14px 18px; border-radius: 10px; 
                border-left: 4px solid {status_color}; margin-bottom: 14px;">
        <div style="display:flex; justify-content:space-between; flex-wrap:wrap; gap:10px;">
            <span><b>📦 Required:</b> {total_req}</span>
            <span style="color:#27ae60;"><b>🚚 Shipped:</b> {total_ship}</span>
            <span style="color:#e74c3c;"><b>⏳ Pending:</b> {total_pending}</span>
            <span style="color:{status_color}; font-weight:700;">{pct:.0f}% complete</span>
        </div>
...
def render():
    ...
    with st.expander("➕ **Add New RDC**", expanded=False):
        with st.form("add_rdc", clear_on_submit=True):
            ...
            # Show equipment inputs in a grid
            req_values = {}
            cols = st.columns(2)
